...
def tr_func(image_list, label_list):
    ...
    n_crops_x = tf.math.ceil(tf.shape(img)[0] / (FLAGS.img_size)) 
    n_crops_y = tf.math.ceil(tf.shape(img)[1] / (FLAGS.img_size)) 
    required_x = n_crops_x * FLAGS.img_size
    required_y = n_crops_y * FLAGS.img_size
    required_x = tf.cast(required_x, tf.float32) - tf.cast(tf.shape(img)[0], tf.float32)
    required_y = tf.cast(required_y, tf.float32) - tf.cast(tf.shape(img)[1], tf.float32)
    required_x /= 2.
    required_y /= 2.
    required_x = tf.cast(required_x, tf.int32)
    required_y = tf.cast(required_y, tf.int32)

    border = tf.pad(img, [[required_x, required_x],[required_y, required_y],[0,0]])
    h = tf.random.uniform([1], 0, tf.cast(tf.shape(border)[0] - FLAGS.img_size, tf.float32))
    h = tf.cast(tf.math.ceil(h[0]), tf.int32)
    w = tf.random.uniform([1], 0, tf.cast(tf.shape(border)[1] - FLAGS.img_size, tf.float32))
    w = tf.cast(tf.math.ceil(w[0]), tf.int32)

    img = border[h:h+FLAGS.img_size, w:w+FLAGS.img_size, :]
    ...
    lab_border = tf.pad(lab, [[required_x, required_x],[required_y, required_y],[0,0]])
    lab = lab_border[h:h+FLAGS.img_size, w:w+FLAGS.img_size, :]
...
def main():
    ...
    model = Path_Unet(input_shape=(FLAGS.img_size, FLAGS.img_size, 3))
